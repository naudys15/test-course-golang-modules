...
var diacritics_remove_map = []struct {
	base    string
	letters string
}{
...
func removeDiacritics(str string) string {
	var diacritics_map = map[rune]string{}
	for _, mapping := range diacritics_remove_map {
		letters := mapping.letters
		for _, letter := range letters {
			diacritics_map[letter] = mapping.base
		}
	}
	reg := regexp.MustCompile("[^\u0000-\u007E]")
	return reg.ReplaceAllStringFunc(str, func(a string) string {
		r, _ := utf8.DecodeRuneInString(a)
		if replacement, ok := diacritics_map[rune(r)]; ok {
			return replacement
		}
		return a
	})
...
func removeSpecialChars(str string) string {
	var remove_non_regex = regexp.MustCompile(`[^\p{L}\p{N} ]+`)
	new_str := remove_non_regex.ReplaceAllString(str, "")
	return new_str
}
...
func isPalindrome(text string) (string, bool) {
	...
	new_text = strings.ToLower(new_text)
	// Remove all special characters
	new_text = removeSpecialChars(new_text)
	// Remove accents
	new_text = removeDiacritics(new_text)
	for i := (len(new_text) - 1); i >= 0; i-- {
		reverse_text += string(new_text[i])
	}
...
func CheckPalindrome(values []string) ([]string, []string, []bool) {
	var phrases []string
	var results []string
	var checks []bool
	var check_strings []string
	if len(values) == 0 {
